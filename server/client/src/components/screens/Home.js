...
function Home() {
    ...
    return (
        <div className = "Home">
            {
                data.length === 0
                ? <h2 style = {{display : "flex", justifyContent : "center"}}>No posts available.</h2>
                : data.map((item) => {
                    return (
                        <div className = "Home__Card card" key = {item._id}>
                            <h5 style = {{fontWeight : "600", padding : "5px"}}><Link to = {item.postedBy._id !== state._id ? "/profile/" + item.postedBy._id : "/profile"}>{item.postedBy.name}</Link>
                                {item.postedBy._id === state._id && <i className = "material-icons likeIcon" style = {{float : "right"}} onClick = {() => deletePost(item._id)}>delete</i>}
                            </h5>
